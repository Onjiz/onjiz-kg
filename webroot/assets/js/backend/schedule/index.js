  ...
  handlerEditEventsByDate(data) {
    ...
    _this.modalSchedule.on('show.bs.modal', function (e) {
      ...
      for (let i = 0; i < dataSchedule.slotSubjects.length; i++) {
        ...
        _.each(listSubjects, function (suje, idx) {
          htmlOption += `<option value="${suje.id}" ${suje.id == dataSchedule.slotSubjects[i].subject.id ? 'selected' : ''}>${suje.title}</option>`
        });
        renderListSubject +=
          `<div data-repeater-item class="row" style="">
              <div class="col-2">
                <div class="form-group">
                  <label>`+ _i18nMsg["TimeStart"] + `</label>
  ...
  initDeleteSchedule() {
    ...
    $('#deleteSchedule').on('click', () => {
      ...
      swal({
        ...
        if (value) {
          Cloud.deleteSchedule.with({ id: id, _csrf:  $('[name="_csrf"]').val() }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
              console.log(err);
              return;
