        ...
        '/*': {
          ...
          fn: async function(req, res, next){
            ...
            if (req.query.branchId) {
              req.session.branchId = req.query.branchId;
            }
            if(!loggedInUser){
              loggedInUser = await Agency.findOne({
                id: req.session.userId
              });
              if(loggedInUser){
                loggedInUser.isAgency = true;
              }
            }
            if (loggedInUser) {
              //check is current logged in user main school admin
              if(loggedInUser.school){
                let schoolObj = await School.findOne({ id: loggedInUser.school });
                loggedInUser.isMainSchoolAdmin = schoolObj && schoolObj.admin == loggedInUser.id ? true : false;
                loggedInUser.schoolObj = schoolObj;
              }
              if (!req.session.branchId) {
                //Get from first brandId of school
                let _listBranches = await Branch.find({ "school": loggedInUser.school, "status": sails.config.custom.STATUS.ACTIVE });
                if (_listBranches.length) {
                  req.session.branchId = _listBranches[0].id;
                }
              }
            } else {
              loggedInUser = await Parent.findOne({ id: req.session.userId });
              loggedInUser.isMainSchoolAdmin = false;
            }

            // If the logged-in user has gone missing, log a warning,
            // wipe the user id from the requesting user agent's session,
            // and then send the "unauthorized" response.
            if (!loggedInUser && req.path.includes('/sa')) {
            ...
            } else if (!loggedInUser && req.path.includes('/backend')) {
              sails.log.warn('Somehow, the user record for the logged-in user (`'+req.session.userId+'`) has gone missing....');
              delete req.session.userId;
              delete req.session.branchId;
              return res.unauthorized();
            } else if (!loggedInUser && req.path.includes('/ag')) {
              sails.log.warn('Somehow, the user record for the logged-in user (`'+req.session.userId+'`) has gone missing....');
              delete req.session.userId;
              return res.unauthorized();
            ...
            //Define web config and send to views
            if (loggedInUser.school) {
              let _webSettings = await SettingService.get({ key: 'web', school: loggedInUser.school }); 
              res.locals.webSettings = _webSettings; 
              if(_webSettings && _webSettings.value && _webSettings.value.language) {
                req.setLocale(_webSettings.value.language);
                sails.hooks.i18n.setLocale(_webSettings.value.language);
                res.cookie("lang-cookie", req.getLocale());
              }
            }
            ...
            req.me = loggedInUser;
            if (loggedInUser.userType == sails.config.custom.TYPE.SCHOOLADMIN) req.me.isMainSchoolAdmin = true;
