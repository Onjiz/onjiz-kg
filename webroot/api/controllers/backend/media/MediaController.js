...
const BackendMediaController = {
	/**
	 @api {put} /Media/add 01. Add a new Media
		...
		*/
	add: async (req, res) => {
		sails.log.info("================================ MediaController.add => START ================================");

		let params = req.allParams();
		if (!params.title) return res.badRequest(ErrorMessages.MEDIA_TITLE_REQUIRED);
		...
		sails.log.info("================================ MediaController.add => END ================================");
	},
	...
	/**
	 @api {put} /Media/get 02. Get detail a Media
		...
		*/
	get: async (req, res) => {
		sails.log.info("================================ MediaController.get => START ================================");
		let params = req.allParams();
	...
	/**
	 @api {put} /Media/edit 04. Edit a Media
		...
		*/
	edit: async (req, res) => {
		sails.log.info("================================ MediaController.edit => START ================================");
	...
	/**
	 @api {put} /Media/trash 04. Edit a Media
		...
		*/
	trash: async (req, res) => {
		sails.log.info("================================ MediaController.trash => START ================================");
	...
	delete: async (req, res) => {
		sails.log.info("================================ MediaController.del => START ================================");
		let params = req.allParams();
		if (!params.id) return res.badRequest(ErrorMessages.MEDIA_ID_REQUIRED);
		let media = await MediaService.get({ id: params.id });
		if (!media) return res.notFound(ErrorMessages.MEDIA_OBJECT_NOT_FOUND);
	...
	search: async (req, res) => {
		sails.log.info("================================ MediaController.search => START ================================");
		let params = req.allParams();
		...
		_.each(arrObjMedia, function (media) {
			...
			tmpData.title = media.title + '<div class="row-action">'
				+ '<a href="javascript:;" class="edit-row" data-id="' + media.id + '">' + sails.__("Edit") + '</a> | '
				+ '<a href="javascript:;" class="duplicate-row" data-id="' + media.id + '">' + sails.__("Duplicate") + '</a> | '
				+ '<a href="javascript:;" class="remove-row" data-id="' + media.id + '">' + sails.__("Trash") + '</a>'
				+ '</div>';
		...
		return res.ok({ draw: draw, recordsTotal: totalMedia, recordsFiltered: totalMedia, data: resMedia });
	},
	...
	*/
	list: async (req, res) => {
		...
		let skip = (parseInt(params.page)) ? (parseInt(params.page) - 1) * parseInt(params.limit) : null;
		let sort = (params.sort) ? JSON.parse(params.sort) : null;
