...
module.exports = {
    ...
    if (params.slotSubjects) {
      for (let _sbj of params.slotSubjects) {
        if(typeof _sbj.subject !== 'object' ) {
          let _sbjObj = await Subject.findOne({id:_sbj.subject });
          if(_sbjObj) {
            let itemSubject = {};
  ...
  get: async (req, res) => {
    ...
    let listSubject = await Subject.find({ where: { school: req.me.school }, sort: [{ title: 'asc' }] });
    let dataSchedule = {};
    ...
    if (!permissions.isMainSchoolAdmin && req.me.role && req.me.role.permissions && req.me.role.permissions.schedule) {
      permissions.isHavePermissionEdit = req.me.role.permissions.schedule.edit ? true : false;
      permissions.isHavePermissionDelete = req.me.role.permissions.schedule.delete ? true : false;
    }
